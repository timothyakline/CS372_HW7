...
TEST_CASE("Basic shapes inheritance", "[baseclass][basic]")
{
        ...
        SECTION("Shape") {
                SECTION("Default Base class init") {
                        ...
                        REQUIRE ( defaultPoly.getHeight() ==  0.0 * (1 + cos(PI
/ 3)) / (2 * sin(PI / 3)) ); REQUIRE ( defaultPoly.getWidth() ==  (0.0 * sin(PI
* (3 - 1) / (2 * 3))) / sin(PI / 3) );
                }
        }
...
TEST_CASE("Post script helper functions") {
        SECTION("Basic commands") {
                ...
                REQUIRE(center() == to_string(CenterX) + " " +
to_string(CenterY) + " moveto\n");
        }
        SECTION("Compound commands") {
                REQUIRE(header() + moveto(30, 50) + rlineto(90, 90) + stroke() +
showpage() == "%!\n\n30 50 moveto\n90 90 rlineto\nstroke\nshowpage\n");
        }
}
...
TEST_CASE("Check doPostScript functions") {
        ...
        auto rotatedTri =
make_unique<RotatedShape>(std::unique_ptr<Triangle>(new Triangle(TEST_LENGTH)),
QUARTER); auto scaledHexa =
make_unique<ScaledShape>(std::unique_ptr<Polygon>(new Polygon(6, TEST_LENGTH)),
0.5, 2.0);
        ...
        fs << moveto(100, 300);
        VerticalShape nonaPentaDeca (move(nona), move(penta),
std::unique_ptr<Polygon>(new Polygon(12, 46.2)));
